...
- 支援站名與地址搜尋（中英文皆可，且不分大小寫）
- 支援全部、有車、無車、無位篩選
- 支援可借最多、可還最多、站名順序、離我最近四種排序
- 點選列表站點後自動移動地圖並開啟 popup
- 支援深色/淺色主題切換，並記住偏好；首次進站會跟隨系統設定
- 支援目前位置定位，並可依距離排序站點
- 每 60 秒自動刷新資料，也可手動重新整理；斷線後會持續自動重試
- 更新失敗時以浮動提示告知，畫面保留前一次的資料
- 手機版地圖在上、列表在下，列表可收合把空間讓給地圖
- 鍵盤快捷鍵：`/` 聚焦搜尋、`Esc` 清空搜尋、`R` 重新整理
- 支援 `prefers-reduced-motion`，會自動關閉動畫效果
